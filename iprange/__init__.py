...
class IPRange():
    def __init__(self, from_file, from_args, verbose=False):
    ...
    def group_ipaddrs(self, ipaddrs, octet):
        group = [ipaddrs[0]]
        for ip in ipaddrs[1:]:
    ...
    def range_ipaddrs(self, ipaddrs):
    ...
    def separate_list(self, from_list, max_len):
        list = []
    ...
    def get_ranged_ports(self, ports):
        unduplicated_ports = list(set(ports))
        duplicated_ports = len(ports) - len(unduplicated_ports)
        if self.verbose and duplicated_ports > 0:
            print(f'found {duplicated_ports} duplicated ports')

        sorted_ports = sorted(unduplicated_ports, key=int)
        for ranged_ports in self.range_ports(ports=sorted_ports):
            yield ranged_ports

    def get_ranged_ipadds(self, ipaddrs):
        unduplicated_ipaddrs = list(set(ipaddrs))
        duplicated_ipaddrs = len(ipaddrs) - len(unduplicated_ipaddrs)
        if self.verbose and duplicated_ipaddrs > 0:
            print(f'found {duplicated_ipaddrs} duplicated ip addresses')

        sorted_ipaddrs = sorted(unduplicated_ipaddrs)
        for grouped_ipaddrs in self.group_ipaddrs(ipaddrs=sorted_ipaddrs, octet=3):
            for ranged_ipaddrs in self.range_ipaddrs(ipaddrs=grouped_ipaddrs):
                yield ranged_ipaddrs

    def ranged_ipaddrs(self, ipaddrs, max_len):
        ranged_ipaddrs_gen = self.get_ranged_ipadds(ipaddrs=ipaddrs)
        if max_len is not None:
            separated_ipaddrs_gen = self.separate_list(from_list=ranged_ipaddrs_gen, max_len=max_len)
            return separated_ipaddrs_gen
        return ranged_ipaddrs_gen

    def ranged_ports(self, ports, max_len):
        ranged_ports_gen = self.get_ranged_ports(ports=ports)
        if max_len is not None:
            separated_ports_gen = self.separate_list(from_list=ranged_ports_gen, max_len=max_len)
